-- Fix volume inversion for zero-fee Swap events.
-- The view must be replaced because CREATE OR REPLACE VIEW cannot change only
-- one expression in an already-created view.
